#include "user.h"

void user::setLambda(const int lambda){
    this->lambda = lambda;
}

void user::setBitLenN(const int bitLenN){
    this->bitLenN = bitLenN;
}


void user::initParams(){
    this->pk.e1 = BN_new();
    this->pk.e2 = BN_new();
    this->pk.N1 = BN_new();
    this->pk.N2 = BN_new();

    this->sk.d1 = BN_new();
    this->sk.d2 = BN_new();
    this->sk.p1 = BN_new();
    this->sk.p2 = BN_new();
    this->sk.q1 = BN_new();
    this->sk.q2 = BN_new();
}

void user::clearParams(){
    BN_clear(this->pk.e1);
    BN_clear(this->pk.e2);
    BN_clear(this->pk.N1);
    BN_clear(this->pk.N2);

    BN_clear(this->sk.d1);
    BN_clear(this->sk.d2);
    BN_clear(this->sk.p1);
    BN_clear(this->sk.p2);
    BN_clear(this->sk.q1);
    BN_clear(this->sk.q2);
}

void user::freeParams(){
    BN_free(this->pk.e1);
    BN_free(this->pk.e2);
    BN_free(this->pk.N1);
    BN_free(this->pk.N2);

    BN_free(this->sk.d1);
    BN_free(this->sk.d2);
    BN_free(this->sk.p1);
    BN_free(this->sk.p2);
    BN_free(this->sk.q1);
    BN_free(this->sk.q2);
}


void user::initMsg(){
    this->msg = BN_new();
}

void user::genMsg(){
    // choose message m
    BN_rand(this->msg, HLEN, BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ANY);
}


void user::printMsg(){
    cout << BN_bn2hex(this->msg) << endl;
}

void user::getMsg(BIGNUM* m){
    BN_copy(m, this->msg);
}

void user::setMsg(BIGNUM* m){
    BN_copy(this->msg, m);
}

void user::freeMsg(){
    BN_free(this->msg);
}

void user::clearMsg(){
    BN_clear(this->msg);
}


void user::initCtx(){
    this->ctx[0] = BN_new();
    this->ctx[1] = BN_new();
    this->ctx[2] = BN_new();
    this->ctx[3] = BN_new();
    this->ctx[4] = BN_new();
}

void user::freeCtx(){
    BN_free(this->ctx[0]);
    BN_free(this->ctx[1]);
    BN_free(this->ctx[2]);
    BN_free(this->ctx[3]);
    BN_free(this->ctx[4]);
}

void user::clearCtx(){
    BN_clear(this->ctx[0]);
    BN_clear(this->ctx[1]);
    BN_clear(this->ctx[2]);
    BN_clear(this->ctx[3]);
    BN_clear(this->ctx[4]);
}


void user::initTd(){
    this->td = BN_new();
}

void user::freeTd(){
    BN_free(this->td);
}

void user::clearTd(){
    BN_clear(this->td);
}



void user::keyGen(){
    BIGNUM* eulerPhi1 = BN_new();
    BIGNUM* eulerPhi2 = BN_new();
    
    BN_CTX* c = BN_CTX_new();
    
    // select p1,q1 and compute N1
    while(BN_num_bits(this->pk.N1) != BLEN){
        BN_generate_prime_ex(this->sk.p1, BLEN/2, 1, NULL, NULL, NULL);
        BN_generate_prime_ex(this->sk.q1, BLEN/2, 1, NULL, NULL, NULL);
        BN_mul(this->pk.N1, this->sk.p1, this->sk.q1, c);
    }

    // select p2,q2 and compute N2
    while(BN_num_bits(this->pk.N2) != BLEN){
        BN_generate_prime_ex(this->sk.p2, BLEN/2, 1, NULL, NULL, NULL);
        BN_generate_prime_ex(this->sk.q2, BLEN/2, 1, NULL, NULL, NULL);
        BN_mul(this->pk.N2, this->sk.p2, this->sk.q2, c);
    }
    
    // compute euler's phi function
    euler_phi(eulerPhi1, this->pk.N1, this->sk.p1, this->sk.q1);
    euler_phi(eulerPhi2, this->pk.N2, this->sk.p2, this->sk.q2);
    
    // set value e 65537, in this source code, omit the computation of coprime(e, eulerPhi)
    BN_set_word(this->pk.e1, 65537);
    BN_set_word(this->pk.e2, 65537);

    // compute d = inverse(e) mod eulerPhi : d*e = 1 mod eulerPhi
    BN_mod_inverse(this->sk.d1, this->pk.e1, eulerPhi1, c);
    BN_mod_inverse(this->sk.d2, this->pk.e2, eulerPhi2, c);

    BN_free(eulerPhi1);
    BN_free(eulerPhi2);

    BN_CTX_free(c);
}

void user::keyGen_rand_e(){   // random e coprime with 
    BIGNUM* eulerPhi1 = BN_new();
    BIGNUM* eulerPhi2 = BN_new();
    BIGNUM* gcd = BN_new();
    BIGNUM* zero = BN_new();
    BN_zero(zero);

    BN_CTX* c = BN_CTX_new();
    
    // select p1,q1 and compute N1
    while(BN_num_bits(this->pk.N1) != BLEN){
        BN_generate_prime_ex(this->sk.p1, BLEN/2, 1, NULL, NULL, NULL);
        BN_generate_prime_ex(this->sk.q1, BLEN/2, 1, NULL, NULL, NULL);
        BN_mul(this->pk.N1, this->sk.p1, this->sk.q1, c);
    }

    // select p2,q2 and compute N2
    while(BN_num_bits(this->pk.N2) != BLEN){
        BN_generate_prime_ex(this->sk.p2, BLEN/2, 1, NULL, NULL, NULL);
        BN_generate_prime_ex(this->sk.q2, BLEN/2, 1, NULL, NULL, NULL);
        BN_mul(this->pk.N2, this->sk.p2, this->sk.q2, c);
    }
    
    // compute euler's phi function
    euler_phi(eulerPhi1, this->pk.N1, this->sk.p1, this->sk.q1);
    euler_phi(eulerPhi2, this->pk.N2, this->sk.p2, this->sk.q2);
    
    // set coprime(e, eulerPhi)
    do{
        do{
            BN_rand(this->pk.e1, BN_num_bits(eulerPhi1), BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ODD);
        }while(!BN_cmp(this->pk.e1, zero) || BN_cmp(eulerPhi1, this->pk.e1) != 1);
        BN_gcd(gcd, eulerPhi1, this->pk.e1, c);
    }while(!BN_is_one(gcd));

    BN_clear(gcd);
    do{
        do{
            BN_rand(this->pk.e2, BN_num_bits(eulerPhi2), BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ODD);
        }while(!BN_cmp(this->pk.e2, zero) || BN_cmp(eulerPhi2, this->pk.e2) != 1);
        BN_gcd(gcd, eulerPhi2, this->pk.e2, c);
    }while(!BN_is_one(gcd));

    // compute d = inverse(e) mod eulerPhi : d*e = 1 mod eulerPhi
    BN_mod_inverse(this->sk.d1, this->pk.e1, eulerPhi1, c);
    BN_mod_inverse(this->sk.d2, this->pk.e2, eulerPhi2, c);

    BN_free(eulerPhi1);
    BN_free(eulerPhi2);
    BN_free(gcd);
    BN_free(zero);

    BN_CTX_free(c);
}

void user::encrypt(BIGNUM* ctx[5], const publicKey pk, const BIGNUM* q){
    if(this->msg == NULL){
        cout << "msg is NULL" << endl;
        abort();
    }

    BIGNUM* r1 = BN_new();
    BIGNUM* r2 = BN_new();

    BIGNUM* zero = BN_new();
    BN_zero(zero);

    //ciphertext & hash
    BIGNUM* h1 = BN_new();
    BIGNUM* h3 = BN_new();
    BIGNUM* h4 = BN_new();
    BIGNUM* h4_inv = BN_new();
    BIGNUM* h5 = BN_new();
    BIGNUM* ray = BN_new();

    BIGNUM* rand_point[2];
    rand_point[0] = BN_new();
    rand_point[1] = BN_new();

    BN_CTX* c = BN_CTX_new();

    // compute point
    sha3_msg(h4, this->msg, q, 4, this->lambda);
    sha3_msg(h5, this->msg, q, 5, this->lambda);
    // compute ray
    BN_mod_inverse(h4_inv, h4, q, c);
    BN_mod_mul(ray, h4_inv, h5, q, c);
    // select random x1
    do{
        BN_rand_range(rand_point[0], q);
    }while(!BN_cmp(rand_point[0], zero));
    // compute y1 on ray
    // y/x * x1 = y1
    BN_mod_mul(rand_point[1], ray, rand_point[0], q, c);

    // select r1, r2
    do{
        BN_rand_range(r1, pk.N1);
        BN_rand_range(r2, pk.N2);
    }while(!BN_cmp(r1, zero) || !BN_cmp(r2, zero));
    
    // C1
    BN_mod_exp(ctx[0], r1, pk.e1, pk.N1, c);

    // C2
    BN_mod_exp(ctx[1], r2, pk.e2, pk.N2, c);
    
    // C3
    BIGNUM* mr2 = BN_new();
    int mr2len = BYTES_HLEN + BYTES_BLEN;
    BN_concat(mr2, this->msg, BYTES_HLEN, r2, BYTES_BLEN);
    sha3(h1, r1, this->lambda);    

    BN_xor(ctx[2], mr2, mr2len, h1, mr2len);

    // C4
    sha3(ctx[3], this->msg, r1, this->lambda);

    // C5
    BIGNUM* xy = BN_new();
    int xylen = BYTES_HLEN * 2;
    BN_concat(xy, rand_point[0], BYTES_HLEN, rand_point[1], BYTES_HLEN);
    sha3(h3, r2, ctx[0], ctx[1], ctx[2], ctx[3], this->lambda);
    BN_xor(ctx[4], xy, xylen, h3, xylen);

    BN_free(r1);
    BN_free(r2);
    BN_free(zero);

    BN_free(h1);
    BN_free(h3);
    BN_free(h4);
    BN_free(h4_inv);
    BN_free(h5);
    BN_free(ray);

    BN_free(mr2);
    BN_free(xy);

    BN_free(rand_point[0]);
    BN_free(rand_point[1]);

    BN_CTX_free(c);
}

void user::decrypt(BIGNUM* result, BIGNUM* ctx[5], const BIGNUM* q){
    BIGNUM* mr2 = BN_new();
    BIGNUM* m_ = BN_new();
    BIGNUM* r1 = BN_new();
    BIGNUM* r2 = BN_new();

    BIGNUM* h1 = BN_new();
    BIGNUM* h2 = BN_new();
    BIGNUM* h3 = BN_new();
    BIGNUM* h4 = BN_new();
    BIGNUM* h4_inv = BN_new();
    BIGNUM* h5 = BN_new();
    BIGNUM* ray = BN_new();

    BIGNUM* xy = BN_new();

    BIGNUM* rand_point_[2];
    rand_point_[0] = BN_new();
    rand_point_[1] = BN_new();

    BIGNUM* ctx2_ = BN_new();
    BIGNUM* ctx4_ = BN_new();

    BN_CTX* c = BN_CTX_new();

    BN_mod_exp(r1, ctx[0], this->sk.d1, this->pk.N1, c);

    sha3(h1, r1, this->lambda);
    int mr2len = BYTES_BLEN + BYTES_HLEN;
    BN_xor(mr2, ctx[2], mr2len, h1, mr2len);

    int m_len = BYTES_HLEN;
    int r2len = BYTES_BLEN;
    BN_parse(m_, r2, mr2, m_len, r2len, mr2len);

    int xylen = BYTES_HLEN * 2;
    sha3(h3, r2, ctx[0], ctx[1], ctx[2], ctx[3], this->lambda);
    BN_xor(xy, ctx[4], xylen, h3, xylen);

    BN_parse(rand_point_[0], rand_point_[1], xy, xylen/2, xylen/2, xylen);

    // compute point
    sha3_msg(h4, m_, q, 4, this->lambda);
    sha3_msg(h5, m_, q, 5, this->lambda);

    // compute ray
    BN_mod_inverse(h4_inv, h4, q, c);
    BN_mod_mul(ray, h4_inv, h5, q, c);

    // check
    BN_mod_exp(ctx2_, r2, this->pk.e2, this->pk.N2, c);
    sha3(ctx4_, m_, r1, this->lambda);

    BIGNUM* y_ = BN_new();
    BN_mod_mul(y_, ray, rand_point_[0], q, c);

    if(BN_cmp(ctx2_, ctx[1]) == 0 && BN_cmp(ctx4_, ctx[3]) == 0 && BN_cmp(rand_point_[1], y_) == 0){
        BN_copy(result, m_);
    }else{
        BN_clear(result);
    }


    BN_free(mr2);
    BN_free(m_);
    BN_free(r1);
    BN_free(r2);

    BN_free(h1);
    BN_free(h2);
    BN_free(h3);
    BN_free(h4);
    BN_free(h4_inv);
    BN_free(h5);
    BN_free(ray);

    BN_free(xy);

    BN_free(rand_point_[0]);
    BN_free(rand_point_[1]);

    BN_free(ctx2_);
    BN_free(ctx4_);
    BN_free(y_);

    BN_CTX_free(c);
}

void user::auth(){
    BN_copy(this->td, this->sk.d2);
}